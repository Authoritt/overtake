...
// The distiller that turns the raw active-notification set into the nav + call snapshots.
...
object CockpitNotifications {
    ...
    fun refresh(sbns: Array<StatusBarNotification>) {
        runCatching { scanNav(sbns) }
            .onFailure { OvertakeLog.w("CockpitNotifications", "scanNav failed", it) }
        runCatching { scanCall(sbns) }
            .onFailure { OvertakeLog.w("CockpitNotifications", "scanCall failed", it) }
    }
     ...
     *
     * Filter: package in {maps, waze}, FLAG_ONGOING_EVENT|FLAG_FOREGROUND_SERVICE, not FLAG_AUTO_CANCEL
